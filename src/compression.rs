...
mod tests {
    ...
    fn test_multiple_large_messages_compression_issue_reproduction() {
        ...
        for i in 1..=10 {
            ...
            println!("Message {}: Original size: {}, Compressed size: {}", 
                     i, csv_like_data.len(), compressed.len());
            ...
            if i >= 2 {
                println!("Successfully processed {} messages without compression errors", i);
            }
        }
    }
    ...
    fn test_github_issue_7_exact_reproduction() {
        ...
        for i in 1..=7 {
            ...
            let compressed = compressor
                ...
                .expect(&format!("GitHub issue: Compression failed on message {}", i));
            
            println!("GitHub issue message {}: Original: {}, Compressed: {}", 
                     i, data_to_send.len(), compressed.len());
    ...
    fn test_extremely_repetitive_data() {
        ...
        for i in 1..=8 {
            ...
            let compressed = compressor
                ...
                .expect(&format!("Repetitive data: Compression failed on message {}", i));
            
            println!("Repetitive message {}: Original: {}, Compressed: {} (ratio: {:.2}%)", 
                     i, repetitive_data.len(), compressed.len(), 
    ...
    fn test_stress_compression_with_mixed_data() {
        ...
        let patterns = vec![
            ...
            (0u8..=255).cycle().take(1000).map(|b| b as char).collect::<String>(),
        ];
        ...
        for (pattern_idx, pattern) in patterns.iter().enumerate() {
            for msg_idx in 1..=5 {
                println!("Stress test pattern {}, message {}", pattern_idx + 1, msg_idx);
                
                let compressed = compressor
                    .compress(pattern.as_bytes())
                    .map_err(|e| {
                        println!("Stress test: Compression error on pattern {} message {}: {}", 
                                pattern_idx + 1, msg_idx, e);
                        e
                    })
                    .expect(&format!("Stress test: Compression failed on pattern {} message {}", 
                ...
                assert_eq!(
                    &decompressed_data[..], 
                    pattern.as_bytes(),
                    "Stress test: Data mismatch on pattern {} message {}", pattern_idx + 1, msg_idx
                );
            }
        }
