...
impl WebSocket {
    ...
    /// allowing for secure WebSocket connections when needed. If a secure connection is anticipated,
    /// a `rustls::TlsConnector` can be optionally provided as the `connector` argument to handle TLS/SSL encryption.
    ///
    /// # Parameters
    /// - `url`: The WebSocket URL to connect to. This can include both `ws` and `wss` schemes.
    /// - `connector`: An optional `TlsConnector` to be used for secure WebSocket (`wss://`) connections.
    ///   If `None` is provided, no additional TLS settings will be applied.
    ///
    /// # Returns
    ...
    ///
    /// This asynchronous function supports both `ws://` (non-secure) and `wss://` (secure) schemes,
    /// allowing for secure WebSocket connections when necessary. Users can provide a set of client-defined
    /// settings through the `Options` parameter, allowing finer control over the WebSocket behavior.
    /// A `rustls::TlsConnector` can optionally be provided as the `connector` argument to enable TLS/SSL
    /// encryption for secure connections.
    ///
    /// # Parameters
    /// - `url`: The WebSocket URL to connect to, supporting both `ws` and `wss` schemes.
    /// - `connector`: An optional `TlsConnector` for establishing secure WebSocket (`wss://`) connections.
    /// - `options`: A set of [`Options`] for configuring connection behavior, such as compression levels, ping intervals, etc.
    ///
    /// # Returns
    /// A `Result` containing either a connected `WebSocket` instance or an error if the connection could not be established.
    ///
    ...
    ///     let ws = WebSocket::connect_with_options(
    ///         url,
    ///         Some(tls_connector()),
    ///         Options::default().with_compression_level(CompressionLevel::fast())
    ///     ).await?;
    ///     // Use `ws` to send and receive messages with specified options
    ///     Ok(())
    /// }
    ///
    /// fn tls_connector() -> TlsConnector {
    ///     todo!() // Set up TLS configuration
    /// }
    /// ```
    ///
    /// # Errors
    /// This function will return an error if:
    /// - The URL provided is invalid or not a WebSocket URL.
    /// - A network or TLS error occurs while attempting to connect.
    ...
    async fn connect_priv(
        ...
        let stream = match scheme {
            ...
            "wss" => {
                let connector = connector.expect("tls connector");
